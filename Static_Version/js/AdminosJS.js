...
$(document).ready(function () {
    ...
    // Top centered menu less than 991px
    if ($(window).width() <= 991) {
    ...
    function sidebarScroll(){
        ...
        $(".mini-body .navbar-static-side").css('overflow-y', 'none!important');
    ...
    $(document).ready(function () {
        $("#search-menu").on("keyup", function () {
            if (this.value.length > 0) {
                $(".navbar-static-side .nav > li.nav-item").hide().filter(function () {
                    return $(this).text().toLowerCase().indexOf($("#search-menu").val().toLowerCase()) != -1;
                }).css("top", "200px").show();
            } else {
                $('.navbar-static-side .nav > li').show();
            }
        });
    });
    //Search In Menu 
...
// Resize canvas when browser resized
